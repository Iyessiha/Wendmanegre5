...
import { FileText, Plus, Printer, Check, Banknote, X, Trash2, ArrowRight, Search } from "lucide-react";
...
export default function FacturationPage() {
  const [type, setType] = useState<TypeDoc>("facture");
  ...
  return (
    <div className="animate-fade-up">
      ...
      <div className="mb-4 flex gap-2">
        {([["facture", "Factures"], ["commande", "Commandes"]] as [TypeDoc, string][]).map(([t, l]) => (
          <button key={t} onClick={() => setType(t)}
            className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-medium transition-colors ${type === t ? "bg-ink text-sand-50" : "bg-white/70 border border-sand-200 text-ink-600 hover:bg-sand-100"}`}>
            <FileText size={15} /> {l}
          </button>
        ))}
      </div>
