...
export function formatXOFCompact(n: number): string {
  if (n >= 1_000_000) return (n / 1_000_000).toFixed(n % 1_000_000 === 0 ? 0 : 1) + " M F";
...
export function statutLabel(s: StatutPret): { label: string; cls: string } {
  switch (s) {
    case "rembourse":
      return { label: "Remboursé", cls: "bg-leaf-100 text-leaf-600" };
    case "partiel":
      return { label: "Partiel", cls: "bg-gold-100 text-clay-700" };
    case "retard":
      return { label: "En retard", cls: "bg-ember-100 text-ember-600" };
    default:
      return { label: "Impayé", cls: "bg-sand-200 text-ink-700" };
  }
}
